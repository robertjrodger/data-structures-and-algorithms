    ...
    def _sort(cls, array):
        ...
        while i <= j:
            ...
            if i < j:
                ...
                break
        cls._swap(array, 0, j)
        sorted_left = cls._sort(array[:j].copy())
        sorted_right = cls._sort(array[j + 1:].copy())
        return cls._concat(sorted_left, array[j], sorted_right)

    @classmethod
    def _swap(cls, array, i, j):
        array[i], array[j] = array[j], array[i]

    @classmethod
    def _concat(cls, left: list, middle: int, right: list):
        concatenated = left
